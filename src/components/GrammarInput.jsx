  ...
  return (
    <div className={`side-panel ${th}`}>
      ...
      <div className="grammar-hint">
        One production per line. Use <code>|</code> for alternatives. Use{" "}
        <code>ε</code> or leave rhs as epsilon symbol for empty productions.
      </div>
      ...
      {parserData?.conflicts?.length > 0 && (
        <div className="conflict-box">
          <div className="conflict-title">
            ⚡ Conflicts Detected ({parserData.conflicts.length})
          </div>
          ...
          {parserData.conflicts.length > 5 && (
            <div className="conflict-item">
              ...and {parserData.conflicts.length - 5} more
            </div>
          )}
        </div>
      )}
      ...
      {parserData && (
        <div className="stats-row">
          <Stat
            label="States"
            value={parserData.canonicalCollection.states.length}
          />
          <Stat label="Terminals" value={parserData.grammar.terminals.size} />
          <Stat
            label="Non-terminals"
            value={parserData.grammar.nonTerminals.size}
          />
          <Stat
            label="Productions"
            value={parserData.grammar.productions.length}
          />
        </div>
      )}
    </div>
